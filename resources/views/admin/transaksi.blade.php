...
<head>
    <meta charset="UTF-8">
    <title>Manajemen Transaksi - Cloth Studio</title>
...
<body class="bg-gray-50">
    ...
    <div class="fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-lg">
        ...
        <nav class="mt-8">
            ...
            <a href="{{ url('/admin/transaksi') }}" class="flex items-center px-6 py-3 bg-gray-100 text-gray-900">
    ...
    <div class="ml-64 p-8">
        <div class="mb-8">
            ...
            <p class="text-gray-600">Kelola semua transaksi pembelian</p>
        </div>
                    ...
                    <tbody>
                        @foreach($transaksi as $trans)
                            <tr class="border-b hover:bg-gray-50">
                                <td class="py-3 px-4">{{ $trans->id }}</td>
                                <td class="py-3 px-4">{{ $trans->username }}</td>
                                <td class="py-3 px-4">Rp {{ number_format($trans->total ?? 0, 0, ',', '.') }}</td>
                                <td class="py-3 px-4">
                                ...
                                <td class="py-3 px-4">
                                    {{ $trans->tanggal_transaksi ? date('d M Y', strtotime($trans->tanggal_transaksi)) : '-' }}
                                </td>
                                <td class="py-3 px-4">
                                    <div class="flex space-x-2">
                                        <a href="{{ url('/admin/transaksi-detail/' . $trans->id) }}" 
                                           class="text-blue-600 hover:text-blue-800">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <button onclick="updateStatus({{ $trans->id }})" 
    ...
    function updateStatus(transaksiId) {
        if (confirm('Update status transaksi ini?')) {
            window.location.href = '{{ url("/admin/update-transaksi") }}?id=' + transaksiId;
        }
    }
    </script>
</body>
